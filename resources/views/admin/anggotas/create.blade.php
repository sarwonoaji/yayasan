...
<div class="container mx-auto px-4 py-8">
    ...
    <form method="POST" action="{{ route('admin.anggotas.store') }}" enctype="multipart/form-data" class="bg-white rounded-lg shadow p-6">
        @csrf
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select name="status_kk" class="w-full px-4 py-2 border rounded @error('status_kk') border-red-500 @enderror" required>
                    ...
                    <option value="Istri" {{ old('status_kk') == 'Istri' ? 'selected' : '' }}>Istri</option>
                    <option value="Anak" {{ old('status_kk') == 'Anak' ? 'selected' : '' }}>Anak</option>
                    <option value="Menantu" {{ old('status_kk') == 'Menantu' ? 'selected' : '' }}>Menantu</option>
                    <option value="Cucu" {{ old('status_kk') == 'Cucu' ? 'selected' : '' }}>Cucu</option>
                    <option value="Orang Tua" {{ old('status_kk') == 'Orang Tua' ? 'selected' : '' }}>Orang Tua</option>
                    <option value="Lainnya" {{ old('status_kk') == 'Lainnya' ? 'selected' : '' }}>Lainnya</option>
                </select>
            ...
            <div>
                <label class="block text-gray-700 font-bold mb-2">Pekerjaan</label>
                    ...
                    <input list="pekerjaan-list" name="pekerjaan_id"
                ...
                @error('pekerjaan_id') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" name="no_telp" class="w-full px-4 py-2 border rounded @error('no_telp') border-red-500 @enderror" 
                    value="{{ old('no_telp') }}" maxlength="20">
            ...
            <div class="col-span-2 mt-6">
                ...
                <div class="grid grid-cols-2 gap-4 mt-3">
                    <div>
                        ...
                        <input type="text" id="lat_preview" class="w-full px-3 py-2 border rounded bg-gray-100" readonly>
                    </div>
                    <div>
                        <label class="text-sm text-gray-600">Longitude</label>
                        <input type="text" id="lng_preview" class="w-full px-3 py-2 border rounded bg-gray-100" readonly>
                    </div>
                </div>
...
<script>
    ...
    const hasLocation = {{ 
        (old('latitude') || $anggota->latitude) ? 'true' : 'false' 
    }};

    const map = L.map('map').setView([initialLat, initialLng], hasLocation ? 15 : 5);
